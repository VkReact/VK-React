...
var VKReact = {
    ...
    shortifyLink: function () {
        ...
        document.getElementById("submitbutton").addEventListener("click", async () => {
            let result = await fetch("http://localhost/vkreact/get_short_link", {
				headers: {
    ...
    showVkReactSettings: function () {
        ...
        if (this.modal_window == 'feed') {
            ...
            <div class="jcat menuitem">
                <div id="jcatcontent" @click="modal_window='feed'">
...
//
    ...
    main: function () { 
        ...
        @font-face {
            font-family: vkmedium;
            src: url('http://localhost/vkreact/vksans_medium');
        }
        @font-face {
            font-family: vkbold;
            src: url('http://localhost/vkreact/vksans_demibold');
        }
        `)
        ...
        async function onUrlSwitch() {
            ...
            if (wall) {
                ...
                if (!document.querySelector("a[href='/verify']")) {
                    document.querySelector(".page_name").appendChild(se(`<a href="/verify" class="page_verified " onmouseover="pageVerifiedTip(this, {type: 1, oid: ${user_id}})"></a>`))
                }
...
VKReact.plugins['feed_disable_ads'] = {
    run: function (context) {
        setInterval(() => {
            if (!window.location.href.endsWith("/feed") || !GM_getValue("feed_disable_ads", false)) return
            ...
            document.querySelectorAll(".wall_marked_as_ads").forEach(it => it.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.remove())
        }, 200)
    },
}
